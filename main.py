...
if __name__ == "__main__":
	...
	# sys.argv.append('transformer_1500_25_9_3.model')
	sys.argv.append('transformer_500_25_20_5.model')
	sys.argv.append('According to the Location Reports of the CC  ,  the ZC ')
	...
	if (method == "train"):
		...
		if is_char_level:
			...
			model.to(device)
			model_opt = NoamOpt(model.src_embed[0].d_model, 1, 400,
			                    torch.optim.Adam(model.parameters(), lr=0.0002, betas=(0.9, 0.98), eps=1e-9))
		...
		if is_char_level:
			...
			model = torch.load(trained_model_name).to(device)
			model.eval()
			dataloader = DataLoader_token(filename, chunk_len, device)
